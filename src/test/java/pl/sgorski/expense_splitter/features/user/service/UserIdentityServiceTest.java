...
public class UserIdentityServiceTest {
    ...
    void isUserIdentityPresent_shouldReturnTrue_whenIdentityExists() {
        when(userIdentityRepository.existsByProviderAndProviderId(any(AuthProvider.class), eq(providerId))).thenReturn(true);

        var exists = userIdentityService.isUserIdentityPresent(providerId, AuthProvider.GOOGLE);

        assertTrue(exists);
        verify(userIdentityRepository, times(1)).existsByProviderAndProviderId(any(AuthProvider.class), eq(providerId));
        verifyNoMoreInteractions(userIdentityRepository);
    }

    @Test
    void isUserIdentityPresent_shouldReturnFalse_whenIdentityDoesNotExist() {
        when(userIdentityRepository.existsByProviderAndProviderId(any(AuthProvider.class), eq(providerId))).thenReturn(false);
    ...
    void findIdentity_shouldReturnUserIdentity_whenIdentityExists() {
        ...
        when(userIdentityRepository.findWithUserByProviderAndProviderId(any(AuthProvider.class), eq(providerId))).thenReturn(Optional.of(identity));

        var found = userIdentityService.findIdentity(AuthProvider.GOOGLE, providerId);

        assertNotNull(found);
        verify(userIdentityRepository, times(1)).findWithUserByProviderAndProviderId(any(AuthProvider.class), eq(providerId));
        verifyNoMoreInteractions(userIdentityRepository);
    }

    @Test
    void findIdentity_shouldThrowIdentityNoTFound_whenIdentityDontExists() {
        when(userIdentityRepository.findWithUserByProviderAndProviderId(any(AuthProvider.class), eq(providerId))).thenReturn(Optional.empty());

        assertThrows(IdentityNotFoundException.class, () -> userIdentityService.findIdentity(AuthProvider.GOOGLE, providerId));
        verify(userIdentityRepository, times(1)).findWithUserByProviderAndProviderId(any(AuthProvider.class), eq(providerId));
        verifyNoMoreInteractions(userIdentityRepository);
    }
