...
public class PasswordChangeRequiredFilterTest {
    ...
    @Test
    void doFilterInternal_shouldPass_whenPasswordChangeRequiredAndPathIsWhitelisted() throws Exception {
        var token = "validToken";
        var header = "Bearer " + token;
    ...
    @Test
    void doFilterInternal_shouldBlock_whenPasswordChangeRequiredAndPathIsNotWhitelisted() throws Exception {
        var token = "validToken";
        var header = "Bearer " + token;
        var requestPath = "/allowed/path";
        when(request.getHeader(anyString())).thenReturn(header);
        ...
        verify(handlerExceptionResolver, times(1)).resolveException(eq(request), eq(response), isNull(), any(PasswordChangeRequiredException.class));
        verify(jwtService, times(1)).isTokenInvalid(eq(token));
        verify(jwtService, times(1)).getPasswordChangeClaim(eq(token));
