...
public final class ExpenseController {
  ...
  public ResponseEntity<Page<ExpenseResponse>> getMyExpenses(Authentication authentication) {
    var result =
        new PageImpl<>(
            List.of(
                new ExpenseResponse(
  ...
  @Operation(
      ...
      description =
          "Retrieves a paginated list of participants for a specific expense and their shares.")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<Page<ExpenseParticipantResponse>> getExpenseParticipants(
    ...
    var user = new UserResponse(UUID.randomUUID(), "user@example.com", Role.USER, Instant.now());
    var result =
        new PageImpl<>(List.of(new ExpenseParticipantResponse(user, BigDecimal.valueOf(333.33))));
    return ResponseEntity.ok(result);
  }
}
