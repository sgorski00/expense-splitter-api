...
public final class FriendshipController {
  ...
  public ResponseEntity<Page<FriendshipResponse>> getMyFriendships(
      Pageable pageable, @RequestParam FriendshipStatus status, Authentication authentication) {
    var user = authenticatedUserResolver.requireUser(authentication);
    var friendships =
        friendshipService
            .getFriendshipsByStatus(user, status, pageable)
  ...
  public ResponseEntity<FriendshipResponse> sendFriendRequest(
      @RequestBody @Valid FriendshipRequest request, Authentication authentication) {
    var user = authenticatedUserResolver.requireUser(authentication);
  ...
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Friendship retrieved successfully.")
      })
  ...
  @ApiResponses(
      value = {@ApiResponse(responseCode = "204", description = "Friendship ended successfully.")})
  public ResponseEntity<Void> endFriendship(@PathVariable UUID id, Authentication authentication) {
    var user = authenticatedUserResolver.requireUser(authentication);
